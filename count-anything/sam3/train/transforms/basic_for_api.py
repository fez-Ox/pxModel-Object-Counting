...
## 新添代码
def _clamp_object_point_(point: torch.Tensor, width: float, height: float) -> torch.Tensor:
    max_x = max(float(width) - 1.0, 0.0)
    max_y = max(float(height) - 1.0, 0.0)
    point[0].clamp_(min=0.0, max=max_x)
    point[1].clamp_(min=0.0, max=max_y)
    return point


def _crop_object_point_(obj, top: float, left: float, height: float, width: float) -> None:
    if obj.point is None:
        return
    obj.point = obj.point.to(dtype=torch.float32)
    obj.point -= torch.as_tensor([left, top], dtype=torch.float32)
    _clamp_object_point_(obj.point, width=width, height=height)


def _hflip_object_point_(obj, width: float) -> None:
    if obj.point is None:
        return
    obj.point = obj.point.to(dtype=torch.float32)
    obj.point[0] = float(width) - obj.point[0]
    obj.point[0].clamp_(min=0.0, max=max(float(width) - 1.0, 0.0))


def _resize_object_point_(obj, ratio_width: float, ratio_height: float) -> None:
    if obj.point is None:
        return
    obj.point = obj.point.to(dtype=torch.float32)
    obj.point *= torch.as_tensor([ratio_width, ratio_height], dtype=torch.float32)


def _pad_object_point_(obj, pad_left: float, pad_top: float) -> None:
    if obj.point is None:
        return
    obj.point = obj.point.to(dtype=torch.float32)
    obj.point += torch.as_tensor([pad_left, pad_top], dtype=torch.float32)


def _apply_metadata_affine_(datapoint, index: int, *, scale_xy=None, offset_xy=None) -> None:
    for query in datapoint.find_queries:
        if query.image_id != index or query.inference_metadata is None:
            continue

        scale_x, scale_y = query.inference_metadata.processed_scale_xy
        offset_x, offset_y = query.inference_metadata.processed_offset_xy
        ...
        if scale_xy is not None:
            mul_x, mul_y = float(scale_xy[0]), float(scale_xy[1])
            scale_x *= mul_x
            scale_y *= mul_y
            offset_x *= mul_x
            offset_y *= mul_y
        ...
        query.inference_metadata.processed_scale_xy = (scale_x, scale_y)
        query.inference_metadata.processed_offset_xy = (offset_x, offset_y)


def _normalize_object_point_(obj, width: float, height: float) -> None:
    if obj.point is None:
        return
    obj.point = obj.point.to(dtype=torch.float32)
    obj.point = obj.point / torch.tensor([width, height], dtype=torch.float32)
## 新添代码


def _sample_offset(extra: int, randomize: bool) -> int:
    if extra <= 0:
        return 0
    if randomize:
        return random.randint(0, extra)
    return extra // 2


def _resolve_scale_with_bounds(
    width: float,
    height: float,
    sampled_scale: float,
    ...
    cur_short = max(float(min(width, height)), 1.0)
    cur_long = max(float(max(width, height)), 1.0)
    min_allowed = float(min_short_side) / cur_short
    max_allowed = float(max_long_side) / cur_long
    if min_allowed <= max_allowed:
        scale = min(max(sampled_scale, min_allowed), max_allowed)
        return scale, False
    # Extremely elongated images may not admit a scale that satisfies both constraints.
    # Prefer respecting the longest-side cap to avoid unexpectedly huge tensors.
    return max_allowed, True


def _object_survives_crop_(obj, top: int, left: int, height: int, width: int) -> bool:
    right = float(left + width)
    bottom = float(top + height)
    if obj.point is not None:
        x = float(obj.point[0].item())
        y = float(obj.point[1].item())
        return float(left) <= x < right and float(top) <= y < bottom
    if obj.bbox is not None:
        bbox = obj.bbox.view(-1, 4)[0]
        x1, y1, x2, y2 = [float(v.item()) for v in bbox]
        inter_w = min(x2, right) - max(x1, float(left))
        inter_h = min(y2, bottom) - max(y1, float(top))
        return inter_w > 0.0 and inter_h > 0.0
    return False


def _filter_cropped_objects_and_queries_(datapoint, index: int, keep_old_ids: list[int]) -> None:
    keep_set = set(keep_old_ids)
    old_to_new = {}
    new_objects = []
    for old_id, obj in enumerate(datapoint.images[index].objects):
        if old_id in keep_set:
            old_to_new[old_id] = len(new_objects)
            new_objects.append(obj)
    datapoint.images[index].objects = new_objects

    for query in datapoint.find_queries:
        if query.image_id != index:
            continue
        query.object_ids_output = [
            old_to_new[old_obj_id]
            for old_obj_id in query.object_ids_output
            if old_obj_id in old_to_new
        ]
...
def crop(
    ...
    _apply_metadata_affine_(
        datapoint,
        index,
        offset_xy=(-float(j), -float(i)),
    )
    ...
    for obj in datapoint.images[index].objects:
        ...
        ## 新添代码
        # 步骤一：object-level GT point 必须和图像裁剪保持同一绝对坐标系。
        # 步骤二：先做平移，再裁到当前裁剪图像边界内。
        # 步骤三：即使当前 object 没有 bbox，point 也仍然可以独立存在。
        _crop_object_point_(obj, top=i, left=j, height=h, width=w)
        ## 新添代码
...
def hflip(datapoint, index):
    ...
    for obj in datapoint.images[index].objects:
        if obj.bbox is not None:
            ...
            obj.bbox = boxes
        ## 新添代码
        # GT point 和 bbox 一样，需要随水平翻转同步更新。
        _hflip_object_point_(obj, width=w)
        ## 新添代码
        if obj.segment is not None:
...
def resize(datapoint, index, size, max_size=None, square=False, v2=False):
    ...
    _apply_metadata_affine_(
        datapoint,
        index,
        scale_xy=(ratio_width, ratio_height),
    )
    ...
    for obj in datapoint.images[index].objects:
        ...
        ## 新添代码
        # GT point 必须和 resize 后图像保持同一个绝对坐标系。
        _resize_object_point_(obj, ratio_width=ratio_width, ratio_height=ratio_height)
        ## 新添代码
        if obj.segment is not None:
...
def pad(datapoint, index, padding, v2=False):
    ...
    if len(padding) != 2:
        _apply_metadata_affine_(
            datapoint,
            index,
            offset_xy=(float(padding[0]), float(padding[1])),
        )
    ...
    for obj in datapoint.images[index].objects:
        if len(padding) != 2:
            ...
            ## 新添代码
            # 左/上 padding 会平移绝对坐标 point；右/下 padding 不会。
            _pad_object_point_(obj, pad_left=padding[0], pad_top=padding[1])
            ## 新添代码
        if obj.segment is not None:
            if v2:
                if len(padding) == 2:
...
class RandomScaleKeepAspectAPI:
    def __init__(
        self,
        scale_min,
        scale_max,
        ...
        v2=False,
        log_infeasible_warning=True,
    ):
        assert scale_min > 0 and scale_max > 0 and scale_max >= scale_min
        self.scale_min = float(scale_min)
        self.scale_max = float(scale_max)
        self.min_short_side = float(min_short_side)
        self.max_long_side = float(max_long_side)
        self.consistent_transform = consistent_transform
        self.v2 = v2
        self.log_infeasible_warning = log_infeasible_warning
        self._warned_infeasible = False

    def _sample_scale(self, width: int, height: int) -> float:
        sampled_scale = random.uniform(self.scale_min, self.scale_max)
        actual_scale, infeasible = _resolve_scale_with_bounds(
            width=width,
            height=height,
        ...
        if infeasible and self.log_infeasible_warning and not self._warned_infeasible:
            logging.warning(
                "RandomScaleKeepAspectAPI found an image aspect ratio that cannot satisfy "
                "both min_short_side=%s and max_long_side=%s at once; prioritizing the "
                "long-side cap.",
                self.min_short_side,
                self.max_long_side,
            )
    ...
    def _apply_scale(self, datapoint, index: int, scale: float):
        cur_size = (
            datapoint.images[index].data.size()[-2:][::-1]
            if self.v2
            else datapoint.images[index].data.size
    ...
    def __call__(self, datapoint, **kwargs):
        if self.consistent_transform and len(datapoint.images) > 0:
            cur_size = (
                datapoint.images[0].data.size()[-2:][::-1]
                if self.v2
                else datapoint.images[0].data.size
            )
            scale = self._sample_scale(cur_size[0], cur_size[1])
            for i in range(len(datapoint.images)):
                datapoint = self._apply_scale(datapoint, i, scale)
            return datapoint

        for i in range(len(datapoint.images)):
            cur_size = (
                datapoint.images[i].data.size()[-2:][::-1]
                if self.v2
                else datapoint.images[i].data.size
            )
            scale = self._sample_scale(cur_size[0], cur_size[1])
            datapoint = self._apply_scale(datapoint, i, scale)
        return datapoint
...
class CropOrPadToSizeAPI:
    ...
    def _get_current_size(self, datapoint, index: int) -> tuple[int, int]:
        if self.v2:
            return datapoint.images[index].data.size()[-2:][::-1]
        return datapoint.images[index].data.size

    def _sample_plan(self, width: int, height: int) -> dict[str, int]:
        crop_width = min(width, self.size)
        crop_height = min(height, self.size)
        crop_left = _sample_offset(width - crop_width, self.random_crop)
        crop_top = _sample_offset(height - crop_height, self.random_crop)

        pad_width = max(self.size - crop_width, 0)
        pad_height = max(self.size - crop_height, 0)
        pad_left = _sample_offset(pad_width, self.random_pad)
        pad_top = _sample_offset(pad_height, self.random_pad)
    ...
    def _apply_plan(self, datapoint, index: int, plan: dict[str, int]):
        width, height = self._get_current_size(datapoint, index)
        crop_left = int(plan["crop_left"])
        crop_top = int(plan["crop_top"])
        crop_width = int(min(plan["crop_width"], width))
        crop_height = int(min(plan["crop_height"], height))

        if crop_width < width or crop_height < height:
            keep_old_ids = [
                obj_id
                for obj_id, obj in enumerate(datapoint.images[index].objects)
                if _object_survives_crop_(
                    obj,
                    top=crop_top,
                    left=crop_left,
                    height=crop_height,
            ...
            _filter_cropped_objects_and_queries_(datapoint, index, keep_old_ids)
    ...
    def __call__(self, datapoint, **kwargs):
        if self.consistent_transform and len(datapoint.images) > 0:
            width, height = self._get_current_size(datapoint, 0)
            plan = self._sample_plan(width, height)
            for index in range(len(datapoint.images)):
                datapoint = self._apply_plan(datapoint, index, plan)
            return datapoint
        ...
        for img in datapoint.images:
            ...
            for obj in img.objects:
                ...
                if obj.bbox is not None:
                    ...
                    obj.bbox = boxes
                ## 新添代码
                # 步骤一：Normalize 阶段把 GT point 从绝对像素坐标转成归一化坐标。
                # 步骤二：这样 collator / matcher 看到的 GT point 坐标系就和 pred_boxes[..., :2] 一致。
                _normalize_object_point_(obj, width=cur_w, height=cur_h)
                ## 新添代码
