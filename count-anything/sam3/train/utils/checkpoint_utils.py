...
def load_checkpoint(
    ...
    with g_pathmgr.open(path, "rb") as f:
        # 这里读取的是完整训练 checkpoint，不是纯 weights 文件。
        # PyTorch 2.6 默认走 weights_only=True，会拒绝某些训练态对象。
...
def load_checkpoint_and_apply_kernels(
    ...
    with g_pathmgr.open(checkpoint_path, "rb") as f:
        # Stage 2 会从 Stage 1 的完整 checkpoint 中抽取 model state_dict。
        checkpoint = torch.load(f, map_location=map_location, weights_only=False)
...
class ModelOnlyCheckpointInitializer:
    def __init__(
        self,
        checkpoint_path: str,
        ckpt_state_dict_keys: Tuple[str, ...] = ("model",),
        strict: bool = True,
        ...
        self.ckpt_state_dict_keys = ckpt_state_dict_keys
        self.strict = strict
        self.ignore_missing_keys = ignore_missing_keys
        self.ignore_unexpected_keys = ignore_unexpected_keys
        self.checkpoint_kernels = checkpoint_kernels
        self.map_location = map_location

    def __call__(self, model: nn.Module):
        # Stage 2 的初始化只恢复模型权重，不恢复 optimizer / scheduler / step。
        state_dict = load_checkpoint_and_apply_kernels(
            checkpoint_path=self.checkpoint_path,
            checkpoint_kernels=self.checkpoint_kernels,
            ckpt_state_dict_keys=self.ckpt_state_dict_keys,
            map_location=self.map_location,
        )
        return load_state_dict_into_model(
            state_dict=state_dict,
            model=model,
            strict=self.strict,
            ignore_missing_keys=self.ignore_missing_keys,
            ignore_unexpected_keys=self.ignore_unexpected_keys,
        )
