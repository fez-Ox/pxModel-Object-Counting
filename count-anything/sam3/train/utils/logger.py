...
def setup_logging(
    ...
    if rank == 0:
        ...
        logging.info(f"EXPERIMENT_START {start_time}")
...
def emit_plain_log_line(message: str = ""):
    """
    直接向 logger 的底层 stream 写纯文本。
    这样可以插入真正的空行，或输出不带 formatter 的日志段落。
    """
