...
def log_env_variables():
    # 环境变量只保留训练调试最常用的关键信息，避免把整份环境刷到日志里。
    tracked_keys = [
    ...
    tracked_parts = [
        f"{key}={os.environ[key]}" for key in tracked_keys if key in os.environ
    ]
    if tracked_parts:
        logging.info("ENV SUMMARY | " + " | ".join(tracked_parts))
    else:
        logging.info("ENV SUMMARY | no tracked variables found")
...
def get_resume_checkpoint(checkpoint_save_dir):
    ...
    for name in g_pathmgr.ls(checkpoint_save_dir):
        match = re.fullmatch(r"epoch(\d+)\.pt", os.path.basename(name))
        if match is None:
            continue
        epoch_num = int(match.group(1))
        if latest_epoch is None or epoch_num > latest_epoch:
            latest_epoch = epoch_num
            latest_epoch_path = os.path.join(checkpoint_save_dir, name)
