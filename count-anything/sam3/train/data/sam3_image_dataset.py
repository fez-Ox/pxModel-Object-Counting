...
class InferenceMetadata:
    ...
    # Geometric transform from raw image coordinates to processed model-input
    # coordinates. The relationship is:
    #   processed_xy = raw_xy * processed_scale_xy + processed_offset_xy
    processed_scale_xy: Tuple[float, float] = (1.0, 1.0)
    processed_offset_xy: Tuple[float, float] = (0.0, 0.0)
...
class Object:
    ...
    ## 新添代码
    # counting 数据使用实例级 GT point 作为一等公民。
    # 当前数据约定里 point 使用绝对像素坐标，不使用相对坐标。
...
class CustomCocoDetectionAPI(VisionDataset):
    ...
    def load_queries(self, pil_images, annotations, queries, img_metadata):
        ...
        for annotation in annotations:
            ...
            point = None
            if bbox is not None:
                # 对于存在 bbox 的实例，优先使用 bbox center 作为 counting point。
                # 这样后续的 crop / pad / resize 都会基于同一几何来源更新 point，
                # 避免原始 point 与 bbox 在增强后出现不一致。
                point = 0.5 * (bbox[0, :2] + bbox[0, 2:])
                point[0].clamp_(min=0, max=w)
                point[1].clamp_(min=0, max=h)
            elif "point" in annotation and annotation["point"] is not None:
                # 对于 point-only 标注，继续回退到原始点监督。
                point = torch.as_tensor(annotation["point"], dtype=torch.float32).view(2)
                point[0].clamp_(min=0, max=w)
                point[1].clamp_(min=0, max=h)
            ## 新添代码
            segment = None
            ...
            images[image_id].objects.append(
                Object(
                    ...
                    area=annotation["area"] if "area" in annotation else 0.0,
                    point=point,
                    object_id=(
            ...
            find_queries.append(
                FindQueryLoaded(
                    ...
                    inference_metadata=InferenceMetadata(
                        # Keep stable image ids in both train and val so any
                        # cross-epoch metric (for example Hungarian instability)
                        # can key by the actual image instead of collapsing
                        # every training sample to -1.
