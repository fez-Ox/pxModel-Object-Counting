...
def collate_fn_api(
    ...
    for i, data in enumerate(batch):
        ...
        for j, q in enumerate(data.find_queries):
            ...
            for object_id in q.object_ids_output:
                ## 新添代码
                # 步骤一：上游 dataset 已经准备好了实例级 GT point。
                #         若原始标注存在 bbox，则这里的 point 已经被替换成 bbox center；
                #         否则回退到原始 point。
                # 步骤二：如果真实 bbox 缺失，则保留零占位框，并用 has_boxes 标记其无效。
                current_object = data.images[q.image_id].objects[object_id]
                current_bbox = current_object.bbox
                current_point = current_object.point
                if current_point is None:
                    raise ValueError(
                        "Counting GT contract requires every object to provide a point or a bbox."
                    )
                if current_bbox is None:
                    current_bbox = current_point.new_zeros(4)
                    current_out_has_boxes.append(False)
                else:
                    current_out_has_boxes.append(True)
                current_out_points.append(current_point.to(dtype=torch.float32).view(2))
                current_out_boxes.append(current_bbox.to(dtype=torch.float32).view(4))
                ## 新添代码
                current_out_object_ids.append(object_id)
            find_targets[stage_id].points.extend(current_out_points)
            find_targets[stage_id].boxes.extend(current_out_boxes)
            find_targets[stage_id].has_boxes.extend(current_out_has_boxes)
    ...
    for i in range(len(stages)):
        ...
        find_targets[i].points_padded = packed_to_padded_naive(
            find_targets[i].points.view(-1, 2), find_targets[i].num_boxes
        )
        ...
        find_targets[i].has_boxes_padded = packed_to_padded_naive(
            find_targets[i].has_boxes, find_targets[i].num_boxes
        )
