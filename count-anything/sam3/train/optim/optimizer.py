...
def validate_param_group_params(
    ...
    """Check that the param groups are non-overlapping and cover all expected parameters.
    ...
    Args:
        ...
        param_allowlist: If specified, only validate against this subset of model
            parameters. This is required for staged training where the optimizer is
            intentionally constructed from only the active module parameters.
    """
    ...
    assert set.union(*parameters) == model_parameters, (
        "Scheduler generated param_groups must include all expected parameters."
...
def construct_optimizer(
    ...
    if validate_param_groups:
        validate_param_group_params(
            param_groups, model, param_allowlist=param_allowlist
        )
...
class GradientClipper:
    ...
    def __call__(
        ...
        parameter_groups: Optional[Mapping[str, Iterable[nn.Parameter]]] = None,
    ):
        if self.max_norm is None:
            return  # no-op

        if parameter_groups is not None:
            for _, group_params in parameter_groups.items():
                group_params = [
                    parameter
                ...
                nn.utils.clip_grad_norm_(
                    group_params, max_norm=self.max_norm, norm_type=self.norm_type
                )
            return

        assert model is not None, "GradientClipper requires model or parameter_groups"
        nn.utils.clip_grad_norm_(
