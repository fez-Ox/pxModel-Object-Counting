...
def _get_rsc_logits(outputs):
    if "rsc_logits" in outputs:
        return outputs["rsc_logits"]
    return outputs["pred_logits"]


def _get_rsc_boxes(outputs):
    if "rsc_boxes" in outputs:
        return outputs["rsc_boxes"]
    return outputs["pred_boxes"]


def _get_rsc_boxes_xyxy(outputs):
    if "rsc_boxes_xyxy" in outputs:
        return outputs["rsc_boxes_xyxy"]
    return outputs["pred_boxes_xyxy"]
...
def _materialize_matched_target_indices(
    indices, *, num_targets: int, device: torch.device
) -> torch.Tensor:
    tgt_idx = indices[2]
    if tgt_idx is not None:
        return tgt_idx
    return torch.arange(num_targets, device=device, dtype=torch.long)


@torch.no_grad()
def build_effective_soft_ce_target_boxes_xyxy(outputs, targets, indices):
    src_boxes_xyxy = _get_rsc_boxes_xyxy(outputs)[(indices[0], indices[1])]
    packed_target_boxes = targets["boxes_xyxy"].to(
        device=src_boxes_xyxy.device, dtype=src_boxes_xyxy.dtype
    )
    packed_target_points = targets["points"].to(
        device=src_boxes_xyxy.device, dtype=src_boxes_xyxy.dtype
    )
    packed_has_boxes = targets["has_boxes"].to(
        device=src_boxes_xyxy.device, dtype=torch.bool
    )

    if packed_target_boxes.numel() == 0:
        empty_boxes = packed_target_boxes.new_zeros((0, 4))
        empty_mask = packed_has_boxes.new_zeros((0,), dtype=torch.bool)
        empty_idx = torch.empty(0, dtype=torch.long, device=src_boxes_xyxy.device)
        return {
            "src_boxes_xyxy": src_boxes_xyxy,
            "matched_target_boxes_xyxy": empty_boxes,
            "matched_target_has_boxes": empty_mask,
            "matched_pseudo_mask": empty_mask,
            "effective_packed_boxes_xyxy": empty_boxes,
            "pseudo_generated_mask": empty_mask,
            "matched_target_indices": empty_idx,
        }

    tgt_idx = _materialize_matched_target_indices(
        indices, num_targets=packed_target_boxes.shape[0], device=src_boxes_xyxy.device
    )
    effective_packed_boxes = packed_target_boxes.clone()
    pseudo_generated_mask = torch.zeros_like(packed_has_boxes)

    matched_target_has_boxes = packed_has_boxes[tgt_idx]
    matched_pseudo_mask = ~matched_target_has_boxes
    if matched_pseudo_mask.any():
        pseudo_tgt_idx = tgt_idx[matched_pseudo_mask]
        matched_src_boxes = src_boxes_xyxy[matched_pseudo_mask]
        matched_tgt_points = packed_target_points[pseudo_tgt_idx]
        src_wh = (matched_src_boxes[:, 2:] - matched_src_boxes[:, :2]).clamp_min(1e-6)
        half_wh = 0.5 * src_wh
        pseudo_boxes_xyxy = torch.cat(
            [
                matched_tgt_points - half_wh,
                matched_tgt_points + half_wh,
            ],
            dim=-1,
        ).clamp_(0.0, 1.0)
        effective_packed_boxes[pseudo_tgt_idx] = pseudo_boxes_xyxy
        pseudo_generated_mask[pseudo_tgt_idx] = True

    return {
        "src_boxes_xyxy": src_boxes_xyxy,
        "matched_target_boxes_xyxy": effective_packed_boxes[tgt_idx],
        "matched_target_has_boxes": matched_target_has_boxes,
        "matched_pseudo_mask": matched_pseudo_mask,
        "effective_packed_boxes_xyxy": effective_packed_boxes,
        "pseudo_generated_mask": pseudo_generated_mask,
        "matched_target_indices": tgt_idx,
    }
...
class IABCEMdetr(LossWithWeights):
    def __init__(
        ...
        point_soft_ce_knn_k: int = 5,
        point_soft_ce_sigma: float = 0.6,
        point_soft_ce_t_min: float = 0.2,
        point_soft_ce_eps: float = 1e-6,
    ):
        ...
        self.alpha = alpha
        self.target_keys.extend(["boxes_xyxy", "has_boxes", "points", "points_padded"])
        self.no_loss_for_fp_propagation = no_loss_for_fp_propagation
        self.point_soft_ce_knn_k = point_soft_ce_knn_k
        self.point_soft_ce_sigma = point_soft_ce_sigma
        self.point_soft_ce_t_min = point_soft_ce_t_min
        self.point_soft_ce_eps = point_soft_ce_eps
        if self.weak_loss:
        ...
        self._warned_missing_bbox_soft_ce = False
    ...
    def get_loss(self, outputs, targets, indices, num_boxes):
        pred_logits = _get_rsc_logits(outputs)
        assert len(pred_logits.shape) > 2, "Incorrect predicted logits shape"
        assert pred_logits.shape[-1] == 1, "Incorrect predicted logits shape"
        src_logits = pred_logits.squeeze(-1)
        ...
        with torch.no_grad():
            ...
            effective_target_boxes = build_effective_soft_ce_target_boxes_xyxy(
                outputs, targets, indices
            )
            src_boxes_xyxy = effective_target_boxes["src_boxes_xyxy"]
            target_boxes_giou = effective_target_boxes["matched_target_boxes_xyxy"]
            matched_pseudo_mask = effective_target_boxes["matched_pseudo_mask"]

            if matched_pseudo_mask.numel() > 0 and matched_pseudo_mask.any():
                if not self._warned_missing_bbox_soft_ce:
                    logging.warning(
                        "IABCEMdetr softCE: matched targets without bbox found; "
                        "using GT-centered pseudo boxes with matched-pred width/height "
                        "for softCE quality targets."
                    )
                    self._warned_missing_bbox_soft_ce = True

            if src_boxes_xyxy.numel() > 0 and target_boxes_giou.numel() > 0:
                gt_cover = box_ops.fast_diag_box_inter_over_gt_area(
                    src_boxes_xyxy,
                    target_boxes_giou,
                )
                t = (
                    prob[(indices[0], indices[1])] ** self.alpha
                    * gt_cover ** (1 - self.alpha)
                )
        ...
        if self.use_presence:
            ...
            ## 新添代码
            # counting point-only 样本没有真实 bbox 时，boxes_padded 会是零占位框。
            # 因此 presence 不应再用 box 宽高是否大于 0 判断“是否有 GT”。
            # 这里只要 object id 有效，就认为该位置对应一个真实 GT 实例。
            if "points_padded" in targets:
                gt_padded_is_visible = gt_padded_object_ids >= 0
            else:
        ...
        losses = {
            "loss_rsc_cls": loss_bce,
            "rsc_cls_f1": bce_f1,
            # Legacy aliases for older configs/log parsers.
            "loss_ce": loss_bce,
            "ce_f1": bce_f1,
...
class Boxes(LossWithWeights):
    def __init__(
        ...
        self.target_keys.extend(["boxes", "boxes_xyxy", "has_boxes"])
    ...
    def get_loss(self, outputs, targets, indices, num_boxes):
        ...
        pred_boxes = _get_rsc_boxes(outputs)
        pred_boxes_xyxy = _get_rsc_boxes_xyxy(outputs)
        # idx = self._get_src_permutation_idx(indices)
        src_boxes = pred_boxes[(indices[0], indices[1])]
        src_boxes_xyxy = pred_boxes_xyxy[(indices[0], indices[1])]
        target_boxes = (
        ...
        target_has_boxes = (
            targets["has_boxes"]
            if indices[2] is None
            else targets["has_boxes"][indices[2]]
        )

        # Counting datasets may legitimately omit bbox annotations.
        # Those targets are carried through the batch as zero placeholder boxes
        # with has_boxes=False, and must not contribute to bbox / giou loss.
        if target_has_boxes.numel() == 0:
        ...
        valid_mask = target_has_boxes.to(dtype=torch.bool)
        ...
        src_boxes = src_boxes[valid_mask]
        src_boxes_xyxy = src_boxes_xyxy[valid_mask]
        target_boxes = target_boxes[valid_mask]
        target_boxes_giou = target_boxes_giou[valid_mask]

        valid_num_boxes = valid_mask.sum().to(
            device=src_boxes.device, dtype=src_boxes.dtype
        )
        valid_num_boxes = torch.clamp(valid_num_boxes, min=1.0)

        loss_bbox = F.l1_loss(src_boxes, target_boxes, reduction="none")

        losses = {}
        losses["loss_rsc_box"] = loss_bbox.sum() / valid_num_boxes
        loss_giou = 1 - box_ops.fast_diag_generalized_box_iou(
            src_boxes_xyxy, target_boxes_giou
        )
        losses["loss_rsc_giou"] = loss_giou.sum() / valid_num_boxes
        # Legacy aliases for older configs/log parsers.
        losses["loss_bbox"] = losses["loss_rsc_box"]
        losses["loss_giou"] = losses["loss_rsc_giou"]
        return losses


class Points(LossWithWeights):
    def __init__(
        self,
        weight_dict=None,
        compute_aux=True,
    ...
    def get_loss(self, outputs, targets, indices, num_boxes):
        ## 新添代码
        # 步骤一：取预测框前两维，作为当前 counting 分支的点预测。
        # 步骤二：用 Hungarian 匹配结果对齐 GT points。
        # 步骤三：沿用 P2PNet 风格的 MSE point loss，并用 num_boxes 做归一化。
        is_video_grounding = outputs.get("is_video_grounding_batch", False)
        if is_video_grounding and not self.apply_loss_to_det_queries_in_video_grounding:
            indices = _keep_only_trk_queries_in_match_inds(
                indices, Q_det=outputs["Q_det"]
        ...
        pred_boxes = _get_rsc_boxes(outputs)
        src_points = pred_boxes[(indices[0], indices[1])][..., :2]
        target_points = (
            targets["points"] if indices[2] is None else targets["points"][indices[2]]
        )

        if src_points.numel() == 0:
            loss_point = pred_boxes.sum() * 0.0
        else:
            loss_point = F.mse_loss(src_points, target_points, reduction="none")
            loss_point = loss_point.sum() / num_boxes

        return {
            "loss_rsc_point": loss_point,
            # Legacy alias for older configs/log parsers.
