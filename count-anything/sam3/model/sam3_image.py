...
def _add_rsc_aliases(out: Dict, include_aux: bool = True):
    if "pred_logits" in out:
        out["rsc_logits"] = out["pred_logits"]
    if "pred_boxes" in out:
        out["rsc_boxes"] = out["pred_boxes"]
        out["rsc_points"] = out["pred_boxes"][..., :2]
    if "pred_boxes_xyxy" in out:
        out["rsc_boxes_xyxy"] = out["pred_boxes_xyxy"]

    if "pred_logits_o2m" in out:
        out["rsc_logits_o2m"] = out["pred_logits_o2m"]
    if "pred_boxes_o2m" in out:
        out["rsc_boxes_o2m"] = out["pred_boxes_o2m"]
        out["rsc_points_o2m"] = out["pred_boxes_o2m"][..., :2]
    if "pred_boxes_xyxy_o2m" in out:
        out["rsc_boxes_xyxy_o2m"] = out["pred_boxes_xyxy_o2m"]

    if "multi_pred_logits" in out:
        out["multi_rsc_logits"] = out["multi_pred_logits"]
    if "multi_pred_boxes" in out:
        out["multi_rsc_boxes"] = out["multi_pred_boxes"]
        out["multi_rsc_points"] = out["multi_pred_boxes"][..., :2]
    if "multi_pred_boxes_xyxy" in out:
        out["multi_rsc_boxes_xyxy"] = out["multi_pred_boxes_xyxy"]

    if include_aux:
        for aux_out in out.get("aux_outputs", []):
            _add_rsc_aliases(aux_out, include_aux=False)
...
class Sam3Image(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if pdc_branch is None:
            pdc_branch = p2p_branch
        elif p2p_branch is not None and p2p_branch is not pdc_branch:
            raise ValueError("Received both pdc_branch and p2p_branch with different modules.")
        self.pdc_branch = pdc_branch
    ...
    @property
    def p2p_branch(self):
        """Legacy access path. The registered module is `pdc_branch`."""
        return self.pdc_branch
    ...
    def _run_decoder(
        ...
        out["_debug_decoder_hs"] = hs
        out["_debug_reference_boxes"] = reference_boxes
        if dec_presence_out is not None:
            out["_debug_dec_presence_out"] = dec_presence_out
    ...
    def _run_segmentation_heads(
        ...
        if self.segmentation_head is not None:
            ...
            if isinstance(backbone_out["img_batch_all_stages"], torch.Tensor):
                image_size = backbone_out["img_batch_all_stages"].new_tensor(
                    backbone_out["img_batch_all_stages"].shape[-2:]
                )
            else:
                ## 新添代码
                # 当前 Stage 6 只正式服务 image/counting 主线；在非标准调用里若拿不到真实图像张量，
                # 先回退到当前仓库固定使用的 1008 输入分辨率，保证 counting pixel decoder 仍可构建。
                image_size = hs.new_tensor((1008, 1008))
                ## 新添代码
            ...
            # Expose resized input size so SAM3 point matcher can operate in
            # resize-space pixels instead of normalized [0, 1] coordinates.
            out["sam_image_size"] = image_size
    ...
    def _run_pdc_branch(self, out, backbone_out, img_ids):
        if self.pdc_branch is None or "pixel_embed" not in out:
            return

        # The PDC branch consumes the pixel embedding produced by the segmentation
        # feature path. We select the current find-stage image batch so dense
        # reference points align with resized image coordinates.
        img_batch = backbone_out["img_batch_all_stages"]
        if isinstance(img_batch, torch.Tensor):
            selected_images = img_batch[img_ids]
        elif img_ids.numel() == 1:
            selected_images = img_batch[img_ids.item()].unsqueeze(0)
        else:
            selected_images = torch.stack([img_batch[i] for i in img_ids.tolist()])
        pdc_outputs = self.pdc_branch(out["pixel_embed"], selected_images.to(out["pixel_embed"]))
        out.update(pdc_outputs)

    def _run_p2p_branch(self, out, backbone_out, img_ids):
        """Legacy method name for older debug hooks."""
        return self._run_pdc_branch(out, backbone_out, img_ids)

    def _get_best_mask(self, out):
        ## 新添代码
        # 旧的 mask 反馈路径先保留注释，当前 counting 版本已经切断 pred_masks，
        # 因此这里不再支持基于最佳 mask 的后续流程。
        #
        ...
        raise RuntimeError("Mask path has been removed from this SAM3 variant.")
        ## 新添代码
    ...
    def forward_grounding(
        ...
        with torch.profiler.record_function("SAM3Image._run_pdc_branch"):
            self._run_pdc_branch(
                out=out,
                backbone_out=backbone_out,
                img_ids=find_input.img_ids,
            )

        # counting/image 主线在 val 阶段也需要和 train 共用同一套 loss 接口。
        # 因此这里不再把 matching 严格限制在 `self.training=True`。
        # 只要当前调用里带有 GT target 且模型上挂了 matcher，就在前向末尾补齐
        # `out["indices"] / aux_out["indices"]`，这样 val 也能稳定计算 loss。
        if find_target is not None and self.matcher is not None:
            self._compute_matching(out, self.back_convert(find_target))
        return out

    def _postprocess_out(self, out: Dict, multimask_output: bool = False):
        ## 新添代码
        # 当前 counting / image 主线已经不再返回 query 级 mask。
        # 因此这里的 postprocess 语义应理解为：
        # 在评估态若存在多 query 输出，则仅保留分数最高的那一个 query 作为最终检测结果。
        ## 新添代码
        ...
        if not self.training and multimask_output and num_mask_boxes > 1:
            ...
            best_query_idx = out["pred_logits"].argmax(1).squeeze(1)
            batch_idx = torch.arange(len(best_query_idx), device=best_query_idx.device)

            out["pred_logits"] = out["pred_logits"][batch_idx, best_query_idx].unsqueeze(
                1
            )
            out["pred_boxes"] = out["pred_boxes"][batch_idx, best_query_idx].unsqueeze(1)
            out["pred_boxes_xyxy"] = out["pred_boxes_xyxy"][
                batch_idx, best_query_idx
            ].unsqueeze(1)

        _add_rsc_aliases(out, include_aux=False)
        return out
    ...
    def back_convert(self, targets):
        ## 新添代码
        # 步骤一：counting 目标里 points 是一等公民，后续 matcher / loss 会直接消费它。
        # 步骤二：boxes 即使缺失也保留零占位张量，同时额外暴露 has_boxes 标记真实框是否存在。
        # 步骤三：positive_map 仍按目标实例数构造，先维持原始 SAM3 接口兼容。
        target_ref = targets.points if targets.points.numel() > 0 else targets.boxes
        batched_targets = {
            "points": targets.points.view(-1, 2),
            "points_padded": targets.points_padded,
            "boxes": targets.boxes.view(-1, 4),
            "boxes_xyxy": box_cxcywh_to_xyxy(targets.boxes.view(-1, 4)),
            "boxes_padded": targets.boxes_padded,
            "has_boxes": targets.has_boxes,
            "has_boxes_padded": targets.has_boxes_padded,
            "positive_map": target_ref.new_ones(len(targets.boxes), 1),
...
class Sam3ImageOnVideoMultiGPU(Sam3Image):
    ...
    def _build_multigpu_buffer_next_chunk(
        ...
        ## 新添代码
        # 旧的基于 mask 的 NMS 逻辑先保留注释，当前版本已经移除了 pred_masks，
        # 因此这里不再执行 mask-NMS。
        #
        # if run_nms:
