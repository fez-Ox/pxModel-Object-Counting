...
class BatchedFindTarget:
    ...
    ## 新添代码
    # 步骤一：显式保存 GT points，作为 counting 任务的主监督信号。
    # 步骤二：保留 packed / padded 两种表示，便于后续 matcher 和 loss 直接复用。
    points: MyTensor
    points__type = torch.float
    points_padded: MyTensor
    points_padded__type = torch.float
    ## 新添代码
    ...
    ## 新添代码
    # 真实 GT bbox 是否存在。对于 point-only 样本，这里为 False，
    # 对应的 boxes / boxes_padded 仅作为占位张量保留形状契约。
    has_boxes: MyTensor
    has_boxes__type = torch.bool
    has_boxes_padded: MyTensor
    has_boxes_padded__type = torch.bool
    ## 新添代码
...
class BatchedInferenceMetadata:
    ...
    # Geometric transform from raw coordinates to processed model-input
    # coordinates:
    #   processed_xy = raw_xy * processed_scale_xy + processed_offset_xy
    processed_scale_xy: MyTensor
    processed_scale_xy__type = torch.float
    processed_offset_xy: MyTensor
    processed_offset_xy__type = torch.float
