...
class SegmentationHead(nn.Module):
    def __init__(
        ...
        self.no_dec = no_dec
        ## 新添代码
        # 旧的 mask 预测路径先保留注释，当前 counting 版本只保留 pixel decoder 输出，
        # 在 pixel_embed 之后不再继续进入 mask 相关的数据流。
        #
        # if no_dec:
        ...
        ## 新添代码
        # 不再向外暴露 query 级 mask 输出，因此这里不再登记 instance key。
        self.instance_keys = []
        ## 新添代码
        ...
        if self.use_encoder_inputs:
            ...
            if self.act_ckpt:
                if image_size is None:
                    image_size = backbone_visual_feats[0].new_tensor((0, 0))
                ...
                pixel_embed = self.pixel_decoder(
                    backbone_visual_feats, image_size=image_size
                )
        else:
            backbone_feats = [x.to(model_device) for x in backbone_feats]
            pixel_embed = self.pixel_decoder(backbone_feats, image_size=image_size)
    ...
    def forward(
        ...
        **kwargs,
        ...
        ## 新添代码
        # 旧的 mask 输出路径先保留注释，方便之后人工核对删除范围。
        #
        # if self.no_dec:
        ...
        return {"pixel_embed": pixel_embed}
        ## 新添代码
...
class PixelDecoder(nn.Module):
    ...
    def forward(
        self, backbone_feats: List[torch.Tensor], image_size: Optional[torch.Tensor] = None
    ):
        # Assumes backbone features are already projected (C == hidden dim)
        prev_fpn = backbone_feats[-1]
        fpn_feats = backbone_feats[:-1]

        if image_size is None or int(image_size[0].item()) <= 0:
        ...
        ## 新添代码
        # 步骤一：当前 image/counting 主线实际送入 PixelDecoder 的层级是：
        #         `x1 / x2 / es`，其中 `es` 已经替换了原最低分辨率特征。
        # 步骤二：因此这里保留原始 `es -> x2` 的 progressive 融合，再把 `x1` 下采样回 `x2` 分辨率融合。
        # 步骤三：当前实验版本不再继续把该特征插值到 1/8；
        #         直接保留融合后的 `1/7` 特征图，交给 stride-7 的 P2P 分支。
        assert len(fpn_feats) >= 2, "Counting pixel decoder expects x1/x2 plus encoder feature."

        x1 = fpn_feats[0]
        lower_feats = fpn_feats[1:]
        for layer_idx, bb_feat in enumerate(lower_feats[::-1]):
            curr_fpn = bb_feat
            prev_fpn = curr_fpn + F.interpolate(
                prev_fpn, size=curr_fpn.shape[-2:], mode=self.interpolation_mode
            )
        ...
        top_layer_idx = 0 if self.shared_conv else len(lower_feats)
        x1_down = F.interpolate(
            x1,
            size=prev_fpn.shape[-2:],
            mode="bilinear",
            align_corners=False,
        )
        prev_fpn = prev_fpn + x1_down
        prev_fpn = self.conv_layers[top_layer_idx](prev_fpn)
        prev_fpn = F.relu(self.norms[top_layer_idx](prev_fpn))
...
class UniversalSegmentationHead(SegmentationHead):
    ...
    def __init__(
        ...
        ## 新添代码
        # 旧的两个 1x1 conv 先保留注释，当前版本在 pixel_embed 之后直接截断，
        # 不再继续构造 semantic_seg / instance_embeds。
        #
    ...
    def forward(
        ...
        **kwargs,
        ...
        ## 新添代码
        # 旧的 mask head 路径先保留注释，当前 counting 版本在 pixel_embed 之后停止，
        # 不再生成 semantic_seg / instance_embeds / pred_masks。
        #
